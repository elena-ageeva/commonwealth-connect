export function getDisplayName(user) {
  return user["Contact Information"]["Preferred Name"].value !== undefined &&
    user["Contact Information"]["Preferred Name"].show
    ? `${user["Contact Information"]["Preferred Name"].value} ${user["Contact Information"]["Last Name"].value}`
    : `${user["Contact Information"]["First Name"].value} ${user["Contact Information"]["Last Name"].value}`;
}

export function getDistance(user1, user2, unit) {
  const user1Geocode =
    user1["Contact Information"]["Primary Address"].value.geocode;
  const user2Geocode =
    user2["Contact Information"]["Primary Address"].value.geocode;
  const [lat1, lon1] = user1Geocode;
  const [lat2, lon2] = user2Geocode;

  if (lat1 === lat2 && lon1 === lon2) {
    return 0;
  } else {
    const radlat1 = (Math.PI * lat1) / 180;
    const radlat2 = (Math.PI * lat2) / 180;
    const theta = lon1 - lon2;
    const radtheta = (Math.PI * theta) / 180;
    let dist =
      Math.sin(radlat1) * Math.sin(radlat2) +
      Math.cos(radlat1) * Math.cos(radlat2) * Math.cos(radtheta);
    if (dist > 1) {
      dist = 1;
    }
    dist = Math.acos(dist);
    dist = (dist * 180) / Math.PI;
    dist = dist * 60 * 1.1515;
    if (unit === "K") {
      dist = dist * 1.609344;
    }
    if (unit === "N") {
      dist = dist * 0.8684;
    }
    return dist;
  }
}
